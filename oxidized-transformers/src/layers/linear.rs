...
use candle_nn::{init::DEFAULT_KAIMING_NORMAL, Init, Module};
...
pub fn linear(in_dim: usize, out_dim: usize, vb: VarBuilder) -> Result<Linear> {
    ...
    let ws = vb.get_with_init((out_dim, in_dim), "weight", init_ws)?;
    let bound = 1. / (in_dim as f64).sqrt();
    let init_bs = Init::Uniform {
        lo: -bound,
        up: bound,
    };
    let bs = vb.get_with_init(out_dim, "bias", init_bs)?;
